...
| `VLLM_CACHE_ROOT` | vLLM runtime cache root; compose sets `/runtime_assets/vllm-cache`, backed by host `/opt/models/runtime_assets/vllm-cache`. |
| `RUNTIME_ASSETS` | Host directory mounted at `/runtime_assets`; deploy derives it as `${MODEL_ROOT}/runtime_assets` (default `/opt/models/runtime_assets`). This volume stores mutable compiler/runtime caches only; model weights remain in the separate `/cache/huggingface` bind mount. |
| `CUTE_DSL_ARCH` | **Not** `VLLM_*` — CuTeDSL/b12x compile target (`sm_121a` on GB10) |
| `FLASHINFER_WORKSPACE_BASE` | **Not** `VLLM_*`. Compose default `/runtime_assets/flashinfer`, backed by host `/opt/models/runtime_assets/flashinfer`. |
| `TILELANG_CACHE_DIR` | **Not** `VLLM_*`. Compose default `/runtime_assets/tilelang-cache` (host `/opt/models/runtime_assets`). Issue #65: in-image `~/.tilelang/cache` dies on container recreate. |
| `TRITON_CACHE_DIR` | **Not** `VLLM_*`. Compose default `/runtime_assets/triton-cache` (host `/opt/models/runtime_assets`). Issue #117: in-image `~/.triton/cache` dies on container recreate, so known shapes re-JIT mid-serve after every restart — and a compiling rank can stall its TP peer past torch's 600s NCCL watchdog. |
...
| `TORCH_FR_DUMP_TEMP_FILE` | **Not** `VLLM_*`. Compose default `/runtime_assets/nccl-fr/comm_lib_trace_rank_` (host `/opt/models/runtime_assets`; rank id appended). Where flight-recorder dumps land — persisted so a killed pair still leaves per-rank evidence for `torchfrtrace`. The entrypoint creates the directory non-fatally; older torch reads `TORCH_NCCL_DEBUG_INFO_TEMP_FILE` instead. **Filenames are static per rank and torch truncates on every dump** — preservation is the operator's job: archive `comm_lib_trace_rank_*` from **both** nodes together after every dump, before the next poke or timeout overwrites them (`torchfrtrace` needs the complete rank set in one directory; each node's volume only holds its own rank's file). |
